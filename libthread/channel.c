 ...
 * f and n represent the  of the queue pointed to by v.
...
chancreate(int elemsize, int elemcnt) {
	...
	if(elemcnt < 0 || elemsize <= 0)
		return nil;
	c = zalloc(sizeof(Channel) + elemcnt * elemsize, 1);
	c->s = elemcnt;
	c->e = elemsize;
...
repeat:
	...
	for (xa = alts; xa->op; xa++) {
		...
		c = xa->c;
		_threaddebug(DBGREND, "Channel is %lux", c);
	...
	if (a == nil) {
		...
		for (a = alts; a->op; a++) {
			...
			if (a->op == CHANNOP) continue;
			if (a->op == CHANNOBLK) break;
			cp = a->c;
			a->tag = &c;
			for (i = 0; i < 32; i++) {
				if ((cp->qused & (1 << i)) == 0) {
		...
		rendezvouses++;
		unlock(&chanlock);
		if (_threadrendezvous(t, (ulong)&c, 0) == ~0) {
			t->call = Callnil;
			return -1;
		}
		...
		for (i = 0; i < 32; i++) {
			if (
				...
				*xa->tag = c;
				...
				rendezvouses++;
				unlock(&chanlock);
				if (_threadrendezvous(t, (ulong)xa->tag, 0) == ~0) {
					t->call = Callnil;
					return -1;
				}
				t->call = Callnil;
		...
		for (i = 0; i < 32; i++) {
			...
			if (
				...
				*a->tag = c;
				...
				rendezvouses++;
				unlock(&chanlock);
				if (_threadrendezvous(getthread(), (ulong)a->tag, 0) == ~0)
					return -1;
				return 1;
			}
		}
...
retry:
	...
	rendezvouses++;
	unlock(&chanlock);
	if (_threadrendezvous(t, (ulong)&tag, 0) == ~0) {
		t->call = Callnil;
		return -1;
	}
	lock(&chanlock);
