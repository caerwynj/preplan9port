...
#define SPARC_NOOP 0x01000000
	...
	case 5:
		...
		lput(INITTEXT+textsize+0x10000L);			/* vaddr */
		lput(INITTEXT+textsize+0x10000L);			/* paddr */
		...
		lput(0L);			/* .text section header name */
		lput(1L);			/* type = PROGBITS */
		lput(0x04L | 0x02L);		/* flags EXEC ALLOC */
		lput(INITTEXT);			/* addr */
		lput(HEADR);			/* offset */
		lput(textsize);			/* file size */
		lput(0);			/* link */
		...
		lput(0L);			/* .data section header name */
		lput(1L);			/* type = PROGBITS */
		lput(0x01L | 0x02L);		/* flags WRITE ALLOC */
		lput(INITTEXT+textsize+0x10000L); /* addr */
		lput(HEADR+textsize);		/* offset */
		lput(datsize+bsssize);			/* file size */
		lput(0);			/* link */
		lput(0);			/* info */
		lput(0);			/* addr align */
		lput(0);			/* entry size */

		lput(0L);			/* .string tbl section header name */
		lput(3L);			/* type = STRTAB */
		lput(0L);			/* flags */
		lput(0); /* addr */
		lput(HEADR);		/* offset */
