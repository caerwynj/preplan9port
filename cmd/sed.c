...
newfile(enum PTYPE type, char *name)
{
	if (type == P_ARG)
		prog.dat.curr = name;
	else if ((prog.dat.bp = Bopen(name, OREAD)) == 0)
...
getrune(void)
{
	...
	if (prog.type == P_ARG) {
		if ((p = prog.dat.curr) != 0) {
			if (*p) {
				prog.dat.curr += chartorune(&r, p);
				c = r;
			} else {
				c = '\n';	/* fake an end-of-line */
				prog.dat.curr = 0;
			...
			c = -1;
	} else if ((c = Bgetrune(prog.dat.bp)) < 0)
			Bterm(prog.dat.bp);
	return c;
}
...
dechain(void)
{
	...
	for(lptr = ltab; lptr < lab; lptr++) {
		...
		if(lptr->chain) {
			rptr = lptr->chain;
			while(trptr = rptr->re.lb1) {
				rptr->re.lb1 = lptr->address;
				rptr = trptr;
			}
			rptr->re.lb1 = lptr->address;
		}
...
ycomp(SedCom *r)
{
	...
	tsp++;
	if ((rp = r->re.text = (Rune *) malloc(sizeof(Rune)*(highc+2))) == 0)
		quit("Out of memory", 0);
...
executable(SedCom *ipc)
{
	if (ipc->active) {	/* Addr1 satisfied - accept until Addr2 */
		...
		switch(ipc->ad2.type) {
			...
			case A_LINE:	/* Line at end of range? */
				if (lnum <= ipc->ad2.re.line) {
					if (ipc->ad2.re.line == lnum)
						ipc->active = 0;
					return !ipc->negfl;
				}
			...
			case A_RE:	/* Check for matching R.E. */
				if (match(ipc->ad2.re.rp, linebuf))
					ipc->active = 0;
				return !ipc->negfl;
	...
	switch (ipc->ad1.type) {	/* Check first address */
		...
		case A_LINE:			/* Check line number */
			if (ipc->ad1.re.line == lnum) {
				ipc->active = 1;	/* In range */
				return !ipc->negfl;
			}
			break;
		case A_RE:			/* Check R.E. */
			if (match(ipc->ad1.re.rp, linebuf)) {
				ipc->active = 1;	/* In range */
				return !ipc->negfl;
			}
			break;
...
arout(void)
{
	...
	for (aptr = abuf; *aptr; aptr++) {
		if((*aptr)->command == ACOM) {
			for(p1 = (*aptr)->re.text; *p1; p1++ )
				Bputrune(&fout, *p1);
			Bputc(&fout, '\n');
		} else {
			for(s = buf, p1= (*aptr)->re.text; *p1; p1++)
					s += runetochar(s, p1);
			*s = '\0';
