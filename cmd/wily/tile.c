...
tile_free(Tile*tile){
	...
	/* FIXME (mhw) This leaks. I think this should be
	if (tile->body)
		view_delete(tile->body);
	view_delete(tile->tag);
	   but this breaks in text_rmview */
	...
	Tile*parent, Text *tagt, Text*bodyt)
{
	...
	tile->tag = view_new(varfont, true, tagt, tile);
	tile->body = bodyt ? view_new(varfont, false, bodyt, tile): 0;
