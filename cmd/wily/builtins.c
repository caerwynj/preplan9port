...
builtin_font(View *v, char *arg) {
	if (arg && strncmp(arg, "var ", 4) == 0) {
		arg += 4;
		setdefaultfont(&varfont, arg);
	} else if (arg && strncmp(arg, "fix ", 4) == 0) {
		arg += 4;
		setdefaultfont(&fixedfont, arg);
	}
...
builtin(View *v, char *cmd, char *arg)
{
	...
	key.name = cmd;
	c = (Cmd *) bsearch( &key, builtins, sizeof(builtins)/sizeof(Cmd),
				 sizeof(Cmd), 
