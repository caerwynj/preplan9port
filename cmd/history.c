...
main(int argc, char *argv[])
{
	...
	tm = localtime(time(0));
	sprint(buf, "/home/xcs0998/%s/%.4d/", ndump, tm->year+1900);
	if(dirstat(buf, &dir)) {
		if(verb)
			print("mounting dump\n");
/*
...
ysearch(char *file)
{
	...
	for(;;) {
		...
		print("%s %s %ld\n", prtime(dir.mtime), buf, dir.len.length);
		toggle ^= 1;
		started = 1;
	}
}
...
lastbefore(ulong t, char *f, char *b)
{
	...
	for(try=0; try<10; try++) {
		tm = localtime(t);
		sprint(b, "/home/xcs0998/%s/%.4d/%.2d%.2d", ndump,
		...
		for(vers=0;; vers++) {
			sprint(b, "/home/xcs0998/%s/%.4d/%.2d%.2d%d", ndump,
		...
		sprint(b, "/home/xcs0998/%s/%.4d/%.2d%.2d%s", ndump,
			tm->year+1900, tm->mon+1, tm->mday, f);
		if(vers)
			sprint(b, "/home/xcs0998/%s/%.4d/%.2d%.2d%d%s", ndump,
