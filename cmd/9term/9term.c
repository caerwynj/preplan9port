...
static char	*items[] = { "cut", "paste", "snarf", "send",  "plumb", "scroll", 0 };
...
button1(Mouse *m)
{
	...
	if(((m->buttons != oldbuttons) || dclick) && oldbuttons&BUTTON1){
...
*/	case mSCROLL:			
	...
	case mPLUMB:
		if (text->p0 != text->p1) {
			...
			importsnarf(text);
		if (text->snarfed) {
			static Rune plumb[] = {'p','l','u','m','b',' ', 0};
			texthighlight(text, text->length, text->length, F&~D);
			sendrunes(plumb, 6);
			sendrunes(text->snarfed, text->snarflen);
			if (text->snarfed[text->snarflen-1] != '\n')
				sendrunes(nl, 1);
		}
		break;
