...
ev_root(char *root_file, int key_len)
{
	...
	dirstat(root_file, &d);
	if (key_len == 0 && d.length < INDX_BLK-1) 
		return 1;
	if (key_len == 0) 
		return 0;
	if ((d.length/INDX_BLK + 2) * key_len < INDX_BLK-1) 
		return 1;
	return 0;
}
...
ev_ld(char *proto_file, PROTO_INFO **p_ptr)
{
	...
	dirfstat(proto_des, &d);
	if ((load = info = malloc(d.length+1)) == nil
	|| read(proto_des, info, d.length) != d.length) {
		ev_err_no = SYSERR;
		close(proto_des);
		free(info);
		return -1;
	...
	*x_val = 0;
	bcount = 0;
	pi = 0;
	while (pi < NIX-1 && bcount < d.length) {
