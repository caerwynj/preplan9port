...
ch_bin(int key_siz, char *in_file, char *out_file)
{
	...
	dirfstat(BFILDES(input), &d);
	buf2[OFF_MAX] = 0;
	for (; off_addr * (OFF_MAX+key_siz+1) < d.length; off_addr++) {
	...
	int in_len, int out_len, int add_to_root, int num_lev)
{
	...
	dirfstat(BFILDES(f_input), &d);
	for (;(num_entries*INDX_BLK)<d.length;num_entries++) {
	...
	if (Bseek(f_input,d.length-in_len,0)==Beof){
		print("pqgen1: %lld %d\n", d.length, in_len);
		return 0;
	}
	if(Bread(f_input,line,in_len-majorhack)==Beof){
		print("in_file=%s out_file=%s, d.length=%lld in_len=%d\n",
			in_file, out_file, d.length, in_len);
		return 0;
	}
	line[in_len-majorhack]=0;
