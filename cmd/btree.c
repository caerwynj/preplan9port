...
#define	WORDSIZ	4000
char	*filename = "/home/xcs0998/lib/words.sorted";
Biobuf	bout;
Biobuf	bin;
...
Rune	entry[WORDSIZ];
Rune	word[WORDSIZ];
Rune	key[100], orig[100], targ[100];
Rune	latin_fold_tab[] =
{
...
btree(char *filename)
{
	...
	do {
		...
		if(range){
			if((ncomp(key,word)<=0) && (ncomp(word,targ)<=0))
				Bprint(&bout, "%S\n", entry);
		}else if(!exact || !acomp(word, key))
			Bprint(&bout, "%S\n", entry);
