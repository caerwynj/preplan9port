...
static XtResource resources[] = {
	...
	{XtNcursorControl, XtCCursorControl, XtRBoolean, sizeof(Boolean),
		Offset(cursorctl), XtRImmediate, (XtPointer)FALSE},
...
Keyaction(Widget w, XEvent *e, String *p, Cardinal *np)
{
	...
	if(k&0xFF00){
		...
		case XK_Left:
			k = cursorctl? 0x82 : 0x80;	/* Left Arrow */
			break;
		case XK_Right:
			k = cursorctl? 0x83 : 0x80;	/* Right Arrow */
			break;
		case XK_Down:
			k = cursorctl? 0x84 : 0x80;	/* Down Arrow */
			break;
		case XK_Up:
			k = cursorctl? 0x85 : 0x81;	/* Up Arrow */
			break;
		case XK_Home:
			if(cursorctl) k = 0x86;	/* Home */
			break;
		case XK_End:
			if(cursorctl) k = 0x87;	/* End */
			break;
		default:
...
void
GwinSelectionPut(Widget w, char*s)
{
	GwinWidget gw = (GwinWidget)w;
